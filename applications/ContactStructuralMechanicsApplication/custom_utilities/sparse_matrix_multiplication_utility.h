...
public:
        ...
        )
    {
        ...
        std::ptrdiff_t* aux_index1_c = new std::ptrdiff_t[nonzero_values];
        std::ptrdiff_t* aux_index2_c = new std::ptrdiff_t[nonzero_values];
    ...
    // TODO: Finish MatrixMultiplicationRMerge !!!!!
//     /**
...
// 
//     #pragma omp for
//             for(int i = 0; i < static_cast<Idx>(A.size1()); ++i) {
//                 Idx row_beg = A.value_data().begin()[i];
//                 Idx row_end = A.value_data().begin()[i+1];
//                 Idx row_width = 0;
//                 for(Idx j = row_beg; j < row_end; ++j) {
//                     Idx a_col = A.col[j];
//                     row_width += B.value_data().begin()[a_col + 1] - B.value_data().begin()[a_col];
//                 }
...
//         }
// 
//         C.set_size(A.size1(), B.size2());
//         C.value_data().begin()[0] = 0;
// 
// 
//     #pragma omp parallel
//         {
...
// 
//     #pragma omp for
//             for(Idx i = 0; i < static_cast<Idx>(A.size1()); ++i) {
//                 Idx row_beg = A.value_data().begin()[i];
//                 Idx row_end = A.value_data().begin()[i+1];
// 
//                 C.value_data().begin()[i+1] = ProdRowWidth(
//                         A.col + row_beg, A.col + row_end, B.value_data().begin(), B.col,
//                         t_col, t_col + max_row_width, t_col + 2 * max_row_width
//                         );
//             }
//         }
// 
//         std::partial_sum(C.value_data().begin(), C.value_data().begin() + A.size1() + 1, C.value_data().begin());
//         C.set_nonzeros(C.value_data().begin()[A.size1()]);
// 
//     #pragma omp parallel
//         {
//     #ifdef _OPENMP
...
//                         B.value_data().begin(), B.col, B.val,
//                         C.col + C.value_data().begin()[i], C.val + C.value_data().begin()[i],
...
// 
//             c_col1 = MergeRows(
//                     amgcl::math::identity<Val>(), tm1_col, tm1_col + c_col1, tm1_val,
//                     amgcl::math::identity<Val>(), tm2_col, tm2_col + c_col2, tm2_val,
//                     tm3_col, tm3_val
//                     ) - tm3_col;
// 
