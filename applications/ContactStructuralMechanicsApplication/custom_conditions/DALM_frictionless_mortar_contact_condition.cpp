    ...
    ) 
{
    ...
    Matrix var_origin_matrix(TNumNodes, 1);
    MortarUtilities::MatrixValue<Variable<double>, NonHistorical>(master_geometry, AUGMENTED_NORMAL_CONTACT_PRESSURE, var_origin_matrix);
    Matrix var_destination_matrix(TNumNodes, 1);
    MortarUtilities::MatrixValue<Variable<double>, NonHistorical>(slave_geometry, AUGMENTED_NORMAL_CONTACT_PRESSURE, var_destination_matrix);
    const Matrix residual_matrix = prod(rMortarConditionMatrices.MOperator, var_origin_matrix) - prod(rMortarConditionMatrices.DOperator, var_destination_matrix); // TODO: Check the sign
